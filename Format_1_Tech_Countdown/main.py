...
# 🚀 DEVELOPMENT TOGGLE
# ==========================================
TEST_MODE = False  # Set to False for Final 1080p Render
...
# Using Impact for that bold, viral aesthetic
...
# --- NEW: PIL Image Generators for flawless graphics ---
def create_hollow_text(text, font_path, font_size, stroke_width):
    """Draws transparent text with a colored border using Python Native PIL"""
    try:
        ...
        # Fill is completely transparent (Alpha 0), Stroke is Solid Yellow
        draw.text((x_offset, y_offset), text, font=font, fill=(0, 0, 0, 0), stroke_width=stroke_width, stroke_fill=(255, 215, 0, 255))
        return np.array(img)
...
def create_scene(img_path, scene_data, voice_audio, scene_index):
    ...
    # ==========================================
    # FACT BADGE (Hollow Text via PIL)
    # ==========================================
    if scene_index > 0:
        ...
        hollow_text_img = create_hollow_text(spaced_text, BOLD_FONT, font_size, stroke_width)
    ...
    # ==========================================
    # LOGO PLATE (Rounded Corners via PIL)
    # ==========================================
    ...
    if domain:
        ...
        if logo_path:
            ...
            # Use PIL to generate the rounded rectangle!
    ...
    # ==========================================
    # MAIN CAPTION (Centered for Intro, Bottom for Facts)
    # ==========================================
    ...
    # 🎯 IF SCENE 0: Lock text dead-center for Thumbnail. OTHERWISE: Place at bottom.
    if scene_index == 0:
...
async def main():
    try:
        ...
        with open(os.path.join(TARGET_DIR, f"caption_{timestamp}.txt"), "w", encoding="utf-8") as f:
        ...
        for i, scene in enumerate(data['scenes']):
            ...
            final_clips.append(create_scene(img_path, scene, audio, i).with_effects([FadeIn(0.2)]))

        output_file = os.path.join(TARGET_DIR, f"{title_slug}-{timestamp}{'-TEST' if TEST_MODE else ''}.mp4")
